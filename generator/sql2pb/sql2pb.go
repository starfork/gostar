...
func GenerateSchema(db *sql.DB, table, serviceName, pkg, fieldStyle, table_prefix string, ignoreTables, ignoreColumns []string) (*Schema, error) {
	s := &Schema{}
	...
	err = typesFromColumns(s, cols, ignoreTables, ignoreColumns, fieldStyle, table_prefix)
	if nil != err {
		return nil, err
	}
...
func typesFromColumns(s *Schema, cols []Column, ignoreTables, ignoreColumns []string, fieldStyle, table_prefix string) error {
	...
	for _, c := range cols {
		...
		//不需要前缀
		if table_prefix != "" {
			table_name = strings.Replace(c.TableName, table_prefix, "", 1)
		}
		messageName := snaker.SnakeToCamel(table_name)
		//messageName = inflect.Singularize(messageName)

		msg, ok := messageMap[messageName]
		if !ok {
			messageMap[messageName] = &Message{Name: messageName, Comment: c.TableComment, Style: fieldStyle, FileName: strings.ToLower(table_name)}
...
func (s *Schema) String() string {
	...
	buf.WriteString(fmt.Sprintf("package go.park.%s;\n", s.Package))
	buf.WriteString("\n")
	...
	for _, m := range s.Messages {
		...
		//m.Name = strings.Replace(m.Name, "Finance", "", 1)
	...
	for _, m := range s.Messages {
		funcTpl += "\t //-----------------------" + m.Comment + "----------------------- \n"
		funcTpl += "\t rpc Create" + m.Name + "(Create" + m.Name + "Request) returns (" + m.Name + "); \n"
		funcTpl += "\t rpc Update" + m.Name + "(Update" + m.Name + "Request) returns (" + m.Name + "); \n"
		funcTpl += "\t rpc Delete" + m.Name + "(Delete" + m.Name + "Request) returns (Response); \n"
		funcTpl += "\t rpc Read" + m.Name + "(Fetch" + m.Name + "Request) returns ((" + m.Name + ")); \n"
		funcTpl += "\t rpc Fetch" + m.Name + "(Fetch" + m.Name + "Request) returns (" + m.Name + "Response); \n"
	}
	funcTpl = funcTpl + "\n}"
...
func (m *Message) GenRpcAddReqRespMessage(buf *bytes.Buffer) {
	...
	//req
	m.Name = "Create" + mOrginName + "Request"
	curFields := []MessageField{}
	var filedTag int
	for _, field := range m.Fields {
		if isInSlice([]string{"id", "create_time", "update_time", "version", "del_state", "deleted_at"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
...
func (m *Message) GenRpcUpdateReqMessage(buf *bytes.Buffer) {
	...
	m.Name = "Update" + mOrginName + "Request"
	curFields := []MessageField{}
	var filedTag int
	for _, field := range m.Fields {
		if isInSlice([]string{"create_time", "update_time", "version", "del_state", "deleted_at"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
...
// GenRpcFetchReqMessage gen add resp message
func (m *Message) GenRpcFetchReqMessage(buf *bytes.Buffer) {
	mOrginName := m.Name
	mOrginFields := m.Fields

	m.Name = "Fetch" + mOrginName + "Requst"
	curFields := []MessageField{}
	var filedTag = len(curFields)
	for _, field := range m.Fields {
		if isInSlice([]string{"version", "del_state", "deleted_at"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
	...
	curFields = append(curFields,
		MessageField{Typ: "uint32", Name: "page", tag: (filedTag + 1), Comment: "page"},
		MessageField{Typ: "uint32", Name: "limit", tag: (filedTag + 2), Comment: "limit"},
	)
	m.Fields = curFields
	buf.WriteString(fmt.Sprintf("%s\n", m))
	...
	m.Fields = []MessageField{
		{Typ: "int64 ", Name: "count", tag: 1},
		{Typ: "repeated " + mOrginName, Name: "data", tag: 2},
	}
	buf.WriteString(fmt.Sprintf("%s\n", m))
...
func parseColumn(s *Schema, msg *Message, col Column) error {
	...
	case "tinyint", "smallint", "int", "mediumint", "bigint":
		tmp := strings.Split(col.ColumnType, " ") //bigint unsigned。
		fieldType = "int32"
		if typ == "bigint" {
			fieldType = "int64"
		}
